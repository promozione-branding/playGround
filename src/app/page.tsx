...
const ExpandableVideoMarquee = dynamic(
  () =>
    import("./components/Home/ExpandableVideoMarquee").then(
      (m) => m.ExpandableVideoMarquee
    ),
  { ssr: false }
);

const KidsStackingCards = dynamic(
  () => import("./components/Home/KidsStackingCards"),
  { ssr: false }
);

const ClientTestimonials = dynamic(
  () =>
    import("./components/Home/ClientTestimonials").then(
      (m) => m.ClientTestimonials
    ),
  { ssr: false }
);

const AnyProSection = dynamic(
  () =>
    import("./components/Home/AnyProSection").then(
      (m) => m.AnyProSection
    ),
  { ssr: false }
);
...
export default function Home() {
  ...
  // ============================================================
  // OPEN POPUP AT 50% PAGE SCROLL
  // ============================================================

  useEffect(() => {
    const handleScroll = () => {
      // Don't open again after it has already opened once
      ...
      // Open popup when user reaches 50%
  ...
  return (
    <main className="min-h-screen bg-white text-[#2D3436]">
      ...
      <div className="hidden h-16 bg-white md:block md:h-24" />
      ...
      {/* ============================================================
          AUTO ENQUIRY POPUP
          Opens once when visitor reaches 50% of page
      ============================================================ */}
